    ...
    V: EngineValidator<T, Block = N::Block>,
{
    ...
    fn on_new_payload(
        ...
        debug!(target: "engine::tree", sync_status=self.backfill_sync_state.is_idle(), "on_new_payload backfill sync status");
        let status = if self.backfill_sync_state.is_idle() {
            let mut latest_valid_hash = None;
            let num_hash = block.num_hash();
            match self.insert_block(block) {
                Ok(status) => {
                    debug!(target: "engine::tree", status=?status, "Insert block status");
                    let status = match status {
                        InsertPayloadOk::Inserted(BlockStatus::Valid) => {
                            latest_valid_hash = Some(block_hash);
                            let res = self.try_connect_buffered_blocks(num_hash);
                            debug!(target: "engine::tree", res=?res, "try_connect_buffered_blocks result");
                            res?;
            ...
            let res = self.on_insert_block_error(error);
            debug!(target: "engine::tree", res=?res, "on_insert_block_error result");
            res?
        } else {
            PayloadStatus::from_status(PayloadStatusEnum::Syncing)
        };
        debug!(target: "engine::tree", status=?status, "on_new_payload status");
        ...
        if outcome.outcome.is_valid() && self.is_sync_target_head(block_hash) {
            ...
            debug!(target: "engine::tree", canonical_block_hash=?self.state.tree_state.canonical_block_hash(), "on_new_payload canonical block hash");
